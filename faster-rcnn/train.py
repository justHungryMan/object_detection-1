    ...
    while not should_stop:
        for _, (_, image_batch, _, bboxes_batch, labels_batch) in enumerate(dataloader):
            ...
            if step % num_steps_to_snapshot == 0 or should_stop:
                path_to_checkpoint = model.module.save(path_to_checkpoints_dir, 0, optimizer, scheduler)
...
if __name__ == '__main__':
    def main():
        ...
        path_to_checkpoints_dir = os.path.join(path_to_outputs_dir, 'checkpoints-{:s}'.format(
            backbone_name))
        if not os.path.isdir(path_to_checkpoints_dir):
