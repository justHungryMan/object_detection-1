...
class ExtraLayers(nn.Module):
    ...
    def __init__(self, inplanes):
        ...
        '''
        self.convbnrelu2 = ConvBnReluLayer(512, 256, kernel_size=3, padding=1, stride=2)
        self.convbnrelu3 = ConvBnReluLayer(256, 256, kernel_size=1, padding=1, stride=2)
    ...
    def forward(self, x):
        ...
        '''
        out2 = self.convbnrelu2(out1_2)
        out3 = self.convbnrelu3(out2)
        out = self.avgpool(out3)
        return out1_2, out2, out3, out
...
class ResNetBase(nn.Module):
    ...
    def __init__(self, block, layers, width=1, name='resnet18', num_classes=1000):
        self.inplanes = 64

        widths = [int(round(ch * width)) for ch in [64, 128, 256, 256]]
    ...
    def load_pretrained_layer(self):
        ...
        pretrained_state_dict = model_zoo.load_url(model_urls[self.name])
        pretrained_param_names = list(pretrained_state_dict.keys())
        for i, param in enumerate(param_names):  # excluding conv6 and conv7 parameters
            if param.find('layer4') != -1 or not param in pretrained_param_names:
                continue
...
def resnet_base(depth, width=1, pretrained=False, **kwargs):
    ...
    model = ResNetBase(block_dict[depth], layers_dict[depth], width, name_dict[depth] , **kwargs)
    return model
