...
best_loss = 10000.  # assume a high loss at first
...
def main():
    ...
    if checkpoint is None:
        model = SSD300(n_classes=n_classes, backbone=args.backbone)
        '''
    ...
    for epoch in range(start_epoch, epochs):
        ...
        # Save checkpoint
        save_checkpoint(epoch, epochs_since_improvement, model, optimizer, val_loss, best_loss, is_best, args.backbone)
    ...
    with torch.no_grad():
        ...
        for i, (images, boxes, labels, difficulties) in enumerate(val_loader):
            ...
            if i % print_freq == 0:
                print('[{0}/{1}]\t'
                      'Batch Time {batch_time.val:.3f} ({batch_time.avg:.3f})\t'
                      'Loss {loss.val:.4f} ({loss.avg:.4f})\t'.format(i+1, len(val_loader),
                                                                      batch_time=batch_time,
                                                                      loss=losses))
